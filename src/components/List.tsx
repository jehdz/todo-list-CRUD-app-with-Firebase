...
export default function List() {
   ...
   const { setElementWidth, state, handleStateFilter, handleDelete } = useGlobalContext();
    ...
    setElementWidth(elementTwoRef.current.getBoundingClientRect().width)
     ...
     return (
        <div className="rounded-md bg-white my-4 shadow-lg w-full">
         ...
         <p ref={elementOneRef} className='text-xs'>{state.filter(item => item.status === 'active').length} items left</p>
         <div ref={elementTwoRef} 
            className='flex-1 hidden md:flex justify-center items-center space-x-3 text-sm'>
               ...
               <p className='cursor-pointer'
                  onClick={() => handleStateFilter('active')}>Active
               </p>  
               <p className='cursor-pointer'
                  onClick={() => handleStateFilter('completed')}>Completed
               </p>  
            </div>
